...
const NotificationDropDown = ({notifications}) => {
  const handleClick = (e) => {
    e.preventDefault();
    checkNotifications(sessionStorage.getItem("token"))
  }
  return (
    <div className="dropdown dropdown-center mr-16 "  onClick={(e)=> handleClick(e)}>
      <div tabIndex={0} role="button" className="btn m-1">
        <Bell />
      </div>
            <ul className="dropdown-content menu bg-base-100 rounded-box z-1 w-68 p-2 shadow-sm">
      {notifications?.filter(Boolean).map((not, index) => (
        <div key={index}>
          <div className={`p-2 m-1 flex flex-col ${not?.checked ? "" : "bg-blue-100"} rounded`}>
            <p className='text-xs'>{not?.content ?? "No content"}</p>
            <p className='text-[8px] text-gray-400 ml-auto'>Today 3:52, {not?.createdAt ?? ""}</p>
          </div>
