...
const UploadImage = ({ onImagesChange, maxImages = 3 }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      // Send base64 strings to parent component
      const base64Array = updatedImages.map(img => img.base64);
      onImagesChange(base64Array);
  ...
  const removeImage = () => {
    ...
    const base64Array = updated.map(img => img.base64);
    onImagesChange(base64Array);
  };
        ...
        >
          <div className="flex flex-col items-center space-y-2 text-gray-600">
            ...
            <p className="text-xs text-gray-400 text-center">
              or drag and drop<br />Up to {maxImages} files (SVG, PNG, JPG, GIF)
            </p>
          </div>
          <input
            id="dropzone-file"
          ...
          {images.length < maxImages ? (
            ...
            <div className="w-full h-full relative group">
              <img
                ...
                title={`You can't upload more than ${maxImages} images`}
              />
              {/* Tooltip */}
